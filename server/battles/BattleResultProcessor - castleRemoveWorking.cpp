...
void BattleResultProcessor::battleFinalize(const BattleID & battleID, const BattleResult & result)
{
	...
	if(defendedTown)
	{
		const bool lowerDestroyed = (*battle)->battleGetWallState(EWallPart::BOTTOM_TOWER) == EWallState::DESTROYED;
		const bool upperDestroyed = (*battle)->battleGetWallState(EWallPart::UPPER_TOWER) == EWallState::DESTROYED;

		if(lowerDestroyed && upperDestroyed && defendedTown->hasBuilt(BuildingID::CASTLE))
		{
			gameHandler->razeStructure(defendedTown->id, BuildingID::CASTLE);
		}
	}
